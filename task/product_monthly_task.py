...
class product_monthly(osv.Model):
    ...
    def compute_product_monthly(self, cr, uid, ids, data, context=None):
        pml_id = data['pml_id']
        # TODO 取得当前id的下一个id：
        next_id_sql = """SELECT id FROM product_monthly_line WHERE id > %s ORDER BY id ASC LIMIT 1"""
    ...
    def compute_closed_product_monthly(self, cr, uid, ids, data, context=None):
        # todo 关闭旧的会计产品月结年度
        product_monthly = self.browse(cr, uid, ids[0], context)
